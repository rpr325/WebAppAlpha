...
app.config['MYSQL_DATABASE_DB'] = 'mlbPitchers'
mysql.init_app(app)
...
def index():
    ...
    cursor.execute('SELECT * FROM tblmlbPitchersImport')
...
def record_view(city_id):
    cursor = mysql.get_db().cursor()
    cursor.execute('SELECT * FROM tblmlbPitchersImport WHERE id=%s', pitcher_id)
    result = cursor.fetchall()
...
def form_edit_get(city_id):
    cursor = mysql.get_db().cursor()
    cursor.execute('SELECT * FROM tblmlbPitchersImport WHERE id=%s', city_id)
    result = cursor.fetchall()
...
def form_update_post(city_id):
    ...
    sql_update_query = """UPDATE tblmlbPitchersImport t SET t.fldName = %s, t.fldLat = %s, t.fldLong = %s, t.fldCountry = 
...
def form_insert_post():
    ...
    sql_insert_query = """INSERT INTO tblmlbPitchersImport (fldName,fldLat,fldLong,fldCountry,fldAbbreviation,fldCapitalStatus,fldPopulation) VALUES (%s, %s,%s, %s,%s, %s,%s) """
